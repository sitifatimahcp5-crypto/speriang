...
import { Project, FashionHistoryItem, TTSHistoryItem, SeoContent, Avatar, Page, VideoHistoryItem, Mode } from '../types';
...
function createWavBlob(decodedPcmData: Uint8Array, sampleRate: number = 24000, numChannels: number = 1): Blob {
...
const TtsAudioPlayer: React.FC<{ item: TTSHistoryItem }> = ({ item }) => {
    const [audioUrl, setAudioUrl] = useState<string | null>(null);

    useEffect(() => {
        const pcmData = decode(item.audioBase64);
    ...
    if (!audioUrl) return <div className="h-10 bg-gray-700 rounded-md animate-pulse w-full"></div>;
...
const History: React.FC<HistoryProps> = ({ setCurrentPage }) => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [savedProjects, savedFashionHistory, savedTtsHistory, savedVideoHistory] = await Promise.all([
        dbService.getProjects(),
        dbService.getFashionHistoryItems(),
        dbService.getTTSHistoryItems(),
      ...
      setTtsHistory(savedTtsHistory.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
  ...
  const handleDeleteTtsItem = async (id: number) => {
    try {
        await dbService.deleteTTSHistoryItem(id);
        setTtsHistory(prev => prev.filter(item => item.id !== id));
    } catch (err) {
        console.error("Failed to delete TTS history item:", err);
        setError("Gagal menghapus item suara.");
    }
  };
  ...
  const handleEditTtsItem = (item: TTSHistoryItem) => {
    sessionStorage.setItem('ttsEditData', JSON.stringify(item));
    setCurrentPage('text-to-speech');
  };
  ...
  const renderProjectDetailsModal = () => {
    ...
    const projectTts = ttsHistory.find(t => t.projectId === selectedProject.id);
            ...
            <Card className="w-full max-w-6xl max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
                ...
                {projectTts && (
                    <Card className="p-4 sm:p-6 mt-6 bg-gray-900/50">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-purple-400">Audio Naskah Tersimpan</h3>
                            <button onClick={() => handleCopyToClipboard(projectTts.text, `tts-script-${projectTts.id}`)} className="flex items-center gap-1 text-xs bg-gray-600 hover:bg-gray-500 px-2 py-1 rounded">
                                {copiedPromptId === `tts-script-${projectTts.id}` ? <CheckIcon className="w-3 h-3 text-green-400" /> : <ClipboardIcon className="w-3 h-3" />}
                                {copiedPromptId === `tts-script-${projectTts.id}` ? 'Naskah Disalin' : 'Salin Naskah'}
                            </button>
                        </div>
                        <TtsAudioPlayer item={projectTts} />
                        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-400 mt-2">
                            <span><strong>Suara:</strong> {projectTts.voice}</span>
                            <span><strong>Gaya:</strong> {projectTts.style || 'Default'}</span>
                            <span className="col-span-2"><strong>Kecepatan:</strong> {projectTts.speakingRate ? `${projectTts.speakingRate}x` : '1x'}</span>
                        </div>
                    </Card>
                )}
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'tts') {
        if (ttsHistory.length === 0) return <Card className="p-6 text-center"><p className="text-gray-400">Belum ada riwayat suara yang disimpan.</p></Card>;
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {ttsHistory.map((item) => (
                    <Card key={item.id} className="p-4 space-y-3 flex flex-col group">
                        <button 
                            onClick={() => handleDeleteTtsItem(item.id!)}
                            className="absolute top-3 right-3 z-10 bg-red-600/70 p-1.5 rounded-full text-white hover:bg-red-700 transition-all opacity-0 group-hover:opacity-100"
                        ...
                        <div className="flex-grow space-y-3">
                            <p className="text-gray-300 bg-gray-900 p-2 rounded text-sm h-20 overflow-y-auto">{item.text}</p>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-400">
                                <span><strong>Suara:</strong> {item.voice}</span>
                                <span><strong>Gaya:</strong> {item.style || 'Default'}</span>
                                <span className="col-span-2"><strong>Kecepatan:</strong> {item.speakingRate ? `${item.speakingRate}x` : '1x'}</span>
                            </div>
                        </div>
                        <div className="mt-auto space-y-2">
                             <TtsAudioPlayer item={item} />
                             <button onClick={() => handleEditTtsItem(item)} className="w-full text-center mt-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1.5 rounded-lg transition-colors text-xs">
                                Edit Naskah & Suara
                            </button>
    ...
    <div className="space-y-8">
      ...
      <div className="border-b border-gray-700">
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <button onClick={() => setActiveTab('tts')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'tts' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'}`}>
                Suara
            </button>
