...
const AssetManager: React.FC = () => {
    ...
    const handleGenerateAvatar = async () => {
        ...
        try {
            const imageBase64 = await cancellableRetry(
                // FIX: Pass the selected visual style to the avatar generation service.
                () => avatarService.generateAvatarImage(generatePrompt, avatarVisualStyle),
                {
                    onRetry: (attempt) => setAvatarError(`Gagal, mencoba lagi... (${attempt}/5)`),
                    isCancelled: () => isAvatarCancelledRef.current
                }
            );
            ...
            if (err.message.includes("cancelled")) {
                ...
                setAvatarError('Gagal membuat avatar. Silakan coba lagi.'); 
            }
        } 
    ...
    const handleGenerateLocation = async () => {
        ...
        try {
            const imageBase64 = await cancellableRetry(
                () => locationService.generateLocationImage(locationGeneratePrompt),
                {
                    onRetry: (attempt) => setLocationError(`Gagal, mencoba lagi... (${attempt}/5)`),
                    isCancelled: () => isLocationCancelledRef.current
                }
            );
            ...
            if (err.message.includes("cancelled")) {
                ...
                setLocationError('Gagal membuat lokasi. Silakan coba lagi.'); 
            }
        } 
