...
const TTS_STYLES = [ { value: 'senang', label: 'Senang' }, { value: 'bahagia', label: 'Bahagia' }, { value: 'sedih', label: 'Sedih' }, { value: 'galau', label: 'Galau' }, { value: 'semangat', label: 'Semangat' }, { value: 'males', label: 'Males' }, { value: 'isi sendiri', label: 'Isi Sendiri...' } ];
...
const GeneralProductManager: React.FC<GeneralProductManagerProps> = ({ setCurrentPage }) => {
  ...
  const [selectedTtsStyle, setSelectedTtsStyle] = useState<string>('senang');
  ...
  useEffect(() => {
    if (selectedAvatarId) {
      ...
      if (selectedAvatar?.gender) {
        setSelectedTtsGender(selectedAvatar.gender);
        const voiceMapping = {
          'Pria': 'Pria / Male',
  ...
  const handleGenerateStoryboard = async () => {
    ...
    try {
      ...
      const generatedStoryboardText = await cancellableRetry(
        () => storyboardService.generateStoryboard(product, sceneCount, finalLanguage, promotionStyle, finalHook),
        {
          onRetry: (attempt) => setLoadingText(`Gagal membuat storyboard. Mencoba lagi... (${attempt}/5)`),
      ...
      for (let i = 0; i < generatedStoryboardText.length; i++) {
        ...
        const sceneImage = await cancellableRetry(
          () => storyboardService.composeSceneImage(avatar.imageBase64, product.imageBase64, sceneText.description, location?.imageBase64),
          {
      ...
      if (err.message.includes("cancelled")) {
        ...
        setError("Terjadi kesalahan saat membuat storyboard. Coba lagi.");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGenerateFinalPrompts = async () => {
    ...
    try {
        ...
        const prompts = await cancellableRetry(
          () => storyboardService.generateVeoPrompts(storyboard, finalLanguage, voiceOverStyle, avatar.gender!),
          {
            onRetry: (attempt) => setLoadingText(`Gagal membuat prompt. Mencoba lagi... (${attempt}/5)`),
            isCancelled: () => isCancelledRef.current,
          }
        );
  ...
  const handleGenerateVoiceover = async () => {
    const finalStyle = selectedTtsStyle === 'isi sendiri' ? customTtsStyle : selectedTtsStyle;
        ...
        <div className="space-y-6">
            ...
            {currentStep === 'final' && (
                <>
                    ...
                    <Card className="p-4 sm:p-6 bg-gray-900/50">
                        <h3 className="text-xl font-bold text-purple-400 mb-4">Naskah Lengkap & Buat Suara</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {selectedTtsStyle === 'isi sendiri' && <input type="text" value={customTtsStyle} onChange={e => setCustomTtsStyle(e.target.value)} placeholder="Gaya kustom (Inggris)" className="w-full bg-gray-700 border border-gray-600 rounded-lg p-2.5 text-sm"/>}
                            </div>
                        </div>
