import { ApiKey, Avatar, Product, Project, FashionHistoryItem, Location, TTSHistoryItem, Character, VideoHistoryItem } from '../types';

const DB_NAME = 'SuperAffiliateDB';
// FIX: Incremented DB version to handle schema change for characters and video history.
const DB_VERSION = 10; 
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (event.oldVersion < 10) {
        ...
        if(transaction) {
            ...
            if (dbInstance.objectStoreNames.contains('ttsHistory')) {
                const ttsStore = transaction.objectStore('ttsHistory');
                if (!ttsStore.indexNames.contains('projectId')) {
                    ttsStore.createIndex('projectId', 'projectId', { unique: false });
                }
            }
        }
      }
...
export const dbService = {
  ...
  addTTSHistoryItem: (item: TTSHistoryItem) => saveData<TTSHistoryItem>('ttsHistory', item),
  getTTSHistoryItems: () => getAllData<TTSHistoryItem>('ttsHistory'),
  deleteTTSHistoryItem: (id: number) => deleteData('ttsHistory', id),
  ...
  getTtsByProjectId: (projectId: number): Promise<TTSHistoryItem[]> => {
    return new Promise(async (resolve, reject) => {
        const db = await initDB();
        const transaction = db.transaction('ttsHistory', 'readonly');
